...
class ChannelAccountController extends Controller
{
    ...
    {
        return ChannelAccount::create($request->validate(['channel' => ['required', 'in:whatsapp,email'], 'provider' => ['required', 'string', 'max:80'], 'name' => ['required', 'string', 'max:120'], 'status' => ['sometimes', 'in:connected,disconnected,blocked']]));
    }
